...
import pdb
from internal import configs, math, stepfun, utils
...
class Camera:
    ...
    radial_distortion: float | None = 1.0
    tangential_distortion: float | None = 1.0
    principal_point: Tuple[float, float] = (400.0, 400.0)
    image_size: Tuple[int, int] = (800, 800)
    is_fisheye: bool = False
    world_to_cam: jnp.ndarray | None = None
...
def convert_to_ndc(
    ...
    infinity_ndc = xnp.stack([xmult * dx / dz, ymult * dy / dz, xnp.ones_like(oz)], axis=-1)
...
def rotation_about_axis(degrees, axis=0):
    ...
    rot2x2 = np.array([[np.cos(radians), -np.sin(radians)], [np.sin(radians), np.cos(radians)]])
    r = np.eye(3)
    r[1:3, 1:3] = rot2x2
...
def generate_ellipse_path(
    ...
    lock_up=False,
    relative_to_first_pose=False,
    flip_y=False,
    first_pose=None,
):
    """Generate an elliptical render path based on the given poses."""
    permute_mat = np.eye(3)
    permute_mat[:3, 1] = np.array([0.0, 0.0, 1.0])

    if flip_y:
        permute_mat[:3, 2] = np.array([0.0, -1.0, 0.0])
    else:
        permute_mat[:3, 2] = np.array([0.0, 1.0, 0.0])

    if relative_to_first_pose:
        if first_pose is None:
            first_pose = np.copy(poses[0, :3, :3])
        else:
            first_pose = np.copy(first_pose[:3, :3])

        poses = np.linalg.inv(first_pose)[None] @ poses[..., :3, :4]
        first_pose = first_pose @ permute_mat
        poses = permute_mat[None] @ poses
    else:
        first_pose = np.eye(3)
    ...
    if relative_to_first_pose:
        positions = (first_pose @ positions.T).T

    # Set path's up vector to axis closest to average of input pose up vectors.
    if relative_to_first_pose:
        if flip_y:
            avg_up = first_pose[:3, 2]
        else:
            avg_up = -first_pose[:3, 2]
    else:
...
def generate_interpolated_path(
    ...
    if n_buffer is not None:
        poses, u, n_frames, u_keyframes = remove_buffer_poses(poses, u, n_frames, u_keyframes, n_buffer)
    ...
    if frames_per_colmap is not None:
        ...
        u = np.linspace(np.min(u_keyframes), np.max(u_keyframes), n_frames, endpoint=True)
...
def identify_file_indices(dir_or_text_file, file_names):
    ...
    if not set(subset_names).issubset(file_names):
        ...
        missing_subset_names_threshold = len(subset_names_missing_from_file_names) / len(subset_names)
        if missing_subset_names_threshold > _IDENTIFY_FILE_INDICES_MISSING_FRACTION_ERROR_THRESHOLD:
            raise ValueError(
...
def get_meters_per_colmap_from_calibration_images(config, poses, image_names):
    ...
    if config.render_calibration_keyframes is not None:
        ...
        if indices:
            ...
            colmap_lengths = np.linalg.norm(positions[:, 0] - positions[:, 1], axis=-1)
...
def calibrate_spline_speed(config, poses, image_names):
    """Uses input config to determine a conversion factor for the spline speed."""

    if config.render_spline_meters_per_sec is None:
        return None

    meters_per_colmap = get_meters_per_colmap_from_calibration_images(config, poses, image_names)
...
def create_render_spline_path(
    ...
    def remove_outlier_spline_indices(spline_indices, poses, q_max, q_mult):
    ...
    spline_indices = identify_file_indices(config.render_spline_keyframes, image_names)
    ...
    if config.render_spline_fixed_up:
        ...
        fixed_up_vector = None
    render_poses, frame_timesteps, keyframe_timesteps = generate_interpolated_path(
        keyframes,
...
def intrinsic_matrix(fx, fy, cx, cy, xnp=np):
...
def get_pixtocam(focal, width, height, xnp=np):
...
def pixel_coordinates(width, height, xnp=np):
...
def pixels_to_rays(
    ...
    if camtype in [ProjectionType.FISHEYE, ProjectionType.FISHEYE_EQUISOLID]:
        ...
        r = xnp.sqrt(xnp.sum(xnp.square(camera_dirs_stacked[Ellipsis, :2]), axis=-1))
    ...
    camera_dirs_stacked = matmul(camera_dirs_stacked, xnp.diag(xnp.array([1.0, -1.0, -1.0])))

    # Extract 2D image plane (x, y) coordinates.
    imageplane = camera_dirs_stacked[0, Ellipsis, :2]

    # Apply camera rotation matrices.
    directions_stacked = mat_vec_mul(camtoworlds[Ellipsis, :3, :3], camera_dirs_stacked)
    ...
    look = ((-camtoworlds[..., :3, 2]).reshape(-1, 3) * jnp.ones_like(directions.reshape(-1, 3))).reshape(directions.shape)
    up = ((camtoworlds[..., :3, 1]).reshape(-1, 3) * jnp.ones_like(directions.reshape(-1, 3))).reshape(directions.shape)

    if virtual_camtoworlds is None:
        vcam_look = look
        vcam_up = up
        vcam_origins = origins
    else:
        vcam_look = ((-virtual_camtoworlds[..., :3, 2]).reshape(-1, 3) * jnp.ones_like(directions.reshape(-1, 3))).reshape(directions.shape)
        vcam_up = ((virtual_camtoworlds[..., :3, 1]).reshape(-1, 3) * jnp.ones_like(directions.reshape(-1, 3))).reshape(directions.shape)
        vcam_origins = xnp.broadcast_to(virtual_camtoworlds[Ellipsis, :3, -1], directions.shape)
    ...
    return origins, directions, viewdirs, radii, imageplane, look, up, origins, vcam_look, vcam_up, vcam_origins
...
def points_to_pixels(
    ...
    if camtype != ProjectionType.PERSPECTIVE:
        raise ValueError(f"points_to_pixels only supports perspective projection, " f"not {camtype} mode.")
...
def rays_planes_intersection(z_min, z_max, origins, directions, xnp=np):
...
def modify_rays_with_bbox(rays, corners, xnp=np):
    ...
    t_min, t_max = ray_box_intersection(rays.origins, rays.directions, corners, xnp=xnp)
...
def cast_ray_batch(
    ...
    if virtual_cameras is not None:
        virtual_cameras_idx = gather_cameras(virtual_cameras, cam_idx, xnp=xnp)
        _, virtual_camtoworlds, _ = virtual_cameras_idx
    else:
        virtual_camtoworlds = camtoworlds

    # Compute rays from pixel coordinates.
    origins, directions, viewdirs, radii, imageplane, look, up, origins, vcam_look, vcam_up, vcam_origins = pixels_to_rays(
        pixels.pix_x_int,
        pixels.pix_y_int,
        pixtocams,
        camtoworlds,
        ...
        xnp=xnp,
        virtual_camtoworlds=virtual_camtoworlds,
    )

    # lights
    lights = lights[cam_idx].reshape(origins.shape)

    if z_range is not None:
        t_min, t_max = rays_planes_intersection(z_range[0], z_range[1], origins, directions, xnp)
    ...
    rays = utils.Rays(
        ...
        vcam_look=vcam_look,
        vcam_up=vcam_up,
        vcam_origins=vcam_origins,
        lossmult=pixels.lossmult,
        near=pixels.near,
        far=pixels.far,
        cam_idx=pixels.cam_idx,
        light_idx=pixels.light_idx,
...
def cast_general_rays(
    ...
    ray_args = (ray_args[0], ray_args[0]) + ray_args[1:]
    ...
    ray_kwargs = {
        ...
        "light_idx": broadcast_scalar(1) * light_idx,
    }
...
def gather_features(points, feature_images, feature_cameras):
    ...
    feature_cameras = jax.tree_util.tree_map(lambda x: x.reshape((-1,) + pad_dims + x.shape[1:]), feature_cameras)
...
def plane_sweep_volume(
    ...
    rays = cast_general_rays(t_c2w, t_p2c, height, width, near, far, t_dist, xnp=xnp)
...
def jax_camera_from_tuple(
    ...
    if distortion_params is not None:
        if "k1" in distortion_params and "k2" in distortion_params and "k3" in distortion_params:
            radial_keys = ["k1", "k2", "k3", "k4"]
            radial_distortion = jnp.array([distortion_params[k] for k in radial_keys if k in distortion_params])
    ...
    extrinsic = jnp.concatenate([extrinsic[:3, :4], jnp.array([[0, 0, 0, 1]])], axis=0)
    ...
    camera = camera.replace(world_to_cam=jnp.copy(world_to_cam))
    return camera
...
def tuple_from_jax_camera(
    ...
    world_to_cam = jnp.copy(jax_camera.world_to_cam)
...
def rotation_distance(rotation_mat1, rotation_mat2):
    ...
    orientation_error_deg = jnp.degrees(jnp.linalg.norm(axis_angle1 - axis_angle2, axis=-1))
    return jnp.where(
...
def compute_camera_metrics(cameras_gt, cameras_pred):
    """Computes the metrics between two cameras."""
    orientation_diffs = jax.vmap(rotation_distance)(cameras_pred.orientation, cameras_gt.orientation)
    translation_diffs = jnp.abs(cameras_pred.translation - cameras_gt.translation)
    diffs = {
        "focal_length": jnp.abs(cameras_pred.focal_length - cameras_gt.focal_length),
        "position": jnp.linalg.norm(cameras_pred.position - cameras_gt.position, axis=-1),
    ...
    if cameras_pred.tangential_distortion is not None:
        ...
        for i in range(cameras_pred.tangential_distortion.shape[-1]):
            diffs[f"tangential_distortion_{i}"] = jnp.abs(
                cameras_pred.tangential_distortion[Ellipsis, i] - tangential_distortion_gt[Ellipsis, i]
            )
