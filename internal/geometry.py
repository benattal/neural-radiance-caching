...
from internal import coord, geopoly, grid_utils, math, ref_utils, render, utils
...
gin.config.external_configurable(coord.contract_constant_1, module="coord")
...
gin.config.external_configurable(coord.contract_cube_1_2, module='coord')
...
@gin.configurable
class BaseDensityMLP(nn.Module):
    """A PosEnc MLP."""
    ...
    density_noise: float = 0.0  # Standard deviation of noise added to raw density.
    ...
    use_corrected_normals: bool = False  # If True compute predicted normals.
    ...
    grid_params: ml_collections.FrozenConfigDict = ml_collections.FrozenConfigDict({})

    backfacing_target: str = "normals"
    backfacing_near: float = 2e-1
    use_backfacing_near: bool = False
    ...
    deg_lights: int = 2
    net_depth_extra: int = 2  # The depth of the second part of MLP.
    net_width_extra: int = 64  # The width of the second part of MLP.
    bottleneck_extra: int = 64  # The width of the second part of MLP.
    skip_layer_extra: int = 4  # Add a skip connection to 2nd MLP every N layers.

    def setup(self):
        # Precompute and store (the transpose of) the basis being used.
        self.pos_basis_t = jnp.array(geopoly.generate_basis(self.basis_shape, self.basis_subdivisions)).T

        self.dense_layer = functools.partial(nn.Dense, kernel_init=getattr(jax.nn.initializers, self.weight_init)())

        self.density_dense_layer = self.dense_layer
        self.final_density_dense_layer = self.dense_layer

        # Density layers
        self.density_layers = [self.density_dense_layer(self.net_width) for i in range(self.net_depth)]

        self.output_density_layer = self.final_density_dense_layer(1)

        self.pred_normals_layer = self.dense_layer(3)
        self.direct_rgb_layer = self.dense_layer(self.config.num_rgb_channels)
        self.transient_indirect_layer = self.dense_layer(self.config.n_bins * self.config.num_rgb_channels)

        # Predicted normals
        self.zeros_layer = functools.partial(
            nn.Dense, kernel_init=getattr(jax.nn.initializers, 'zeros')
        )
        self.normals_offset_layer = self.zeros_layer(3)

        # Appearance grid
        if self.use_grid:
            self.grid = grid_utils.GRID_REPRESENTATION_BY_NAME[self.grid_representation.lower()](
                name="density_grid", **self.grid_params
            )
        else:
            self.grid = None
    ...
    def run_network(self, x, means):
        ...
        raw_density = self.output_density_layer(x)[Ellipsis, 0]  # Hardcoded to a single channel.

        return raw_density, x

    def run_extra_network(self, x):
        inputs = x

        # Evaluate network to produce the output density.
        for i in range(self.net_depth_extra):
            x = self.extra_layers[i](x)
            x = self.net_activation(x)

            if i % self.skip_layer_extra == 0 and i > 0:
                x = jnp.concatenate([x, inputs], axis=-1)

        return x

    def get_extra_bottleneck(
        self,
        lights,
        bottleneck,
    ):
        # Run view dependent network
        extra_input = self.lights_enc_fn(lights)
        extra_input = jnp.concatenate(
            [
                bottleneck,
                extra_input,
            ],
        ...
        return self.run_extra_network(extra_input)
    ...
    def predict_density(
        ...
        **kwargs
        ...
        if self.grid is not None:
            ...
            if not self.squash_before and self.warp_fn is not None:
                if perp_mag is not None and self.unscented_scale_mult > 0:
                    if self.warp_fn.__wrapped__ == coord.contract:
                        ...
                        control, perp_mag = coord.track_isotropic(self.warp_fn, control, perp_mag)
            ...
            if not self.use_feature_filter or (not is_secondary and self.use_feature_filter_secondary_only):
                x.append(
                    self.grid(
                        control,
                        x_scale=scale,
                ...
                feature_filter = jnp.linalg.norm(means[..., None, :], axis=-1, keepdims=True) < self.feature_filter_radius

                if self.use_feature_filter_far_field:
                    control = jnp.where(
                        feature_filter,
                        control,
                        self.warp_fn(jnp.ones_like(control) * kwargs['viewdirs'] * 100.0),
                    )
                ...
                x.append(
                    self.grid(
                        ...
                        feature_filter_size=self.feature_filter_size
                    )
                )
        ...
        if (density_key is not None) and (self.density_noise > 0):
            raw_density += self.density_noise * random.normal(density_key, raw_density.shape)
        
        # raw_density = self.convert_raw_density(raw_density, means)
    ...
    def convert_raw_density(self, raw_density, means):
        ...
        if self.grid is not None:
            ...
            density_is_valid = jnp.all(
                (warped_means > self.grid.bbox[0]) & (warped_means < self.grid.bbox[1]),
                axis=-1,
            )
    ...
    def get_predict_density_kwargs(
        ...
        **kwargs,
        ...
        if self.grid is not None:
            ...
            predict_density_kwargs["viewdirs"] = jnp.ones_like(means[..., None, :]) * rays.viewdirs[..., None, None, :]
    ...
    def predict_density_normals(
        ...
        **kwargs,
        ...
        if self.disable_density_normals or mesh_normals is not None:
            ...
            (raw_density_flat, x_flat), raw_grad_density_flat = predict_density_and_grad_fn(
                *gaussians_flat, **pd_kwargs_flat
            )
            ...
            normals = jnp.nan_to_num(-ref_utils.l2_normalize(raw_grad_density))
        ...
        if self.enable_pred_normals:
            ...
            normals_pred = jnp.nan_to_num(-ref_utils.l2_normalize(grad_pred))
            if self.use_corrected_normals:
              normals = jnp.where(
                  math.dot(normals, rays.viewdirs[Ellipsis, None, :]) < 0, normals, -normals
              )
              normals_pred = jnp.where(
                  math.dot(normals_pred, rays.viewdirs[Ellipsis, None, :]) < 0, normals_pred, -normals_pred
              )
        ...
        normals_shading = None

        if self.enable_normals_offset:
            normals_offset = self.normals_offset_layer(x)

            normals_shading = normals_to_use + normals_offset
            normals_shading = ref_utils.l2_normalize(normals_shading)

        lights = rays.lights
        ray_offset = rays.origins[..., None, :] - means 
        ray_dists = jnp.linalg.norm(ray_offset, axis=-1, keepdims=True)

        light_offset = lights[..., None, :] - means
        ...
        return dict(
            ...
            normals_shading=normals_shading,
            ray_dists=ray_dists,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.backfacing_target in sampler_results and sampler_results[self.backfacing_target] is not None:
            dotprod = math.dot(
                sampler_results[self.backfacing_target],
                -rays.directions[Ellipsis, None, :],
            )[Ellipsis, 0]

            if is_secondary and self.use_backfacing_near:
            ...
            #         ((dotprod > self.filter_backfacing_threshold) | (alphas > self.filter_backfacing_alpha_threshold)),
            ...
            #     sampler_results["density_multiplier"] = jax.lax.stop_gradient(sampler_results["density_multiplier"])
...
@gin.configurable
class DensityMLP(BaseDensityMLP):
  pass

@gin.configurable
class TransientDensityMLP(BaseDensityMLP):
  pass