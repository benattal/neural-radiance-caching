...
LPIPS_TFHUB_PATH = '/scratch/ondemand28/anagh/active-yobo/third_party/net-lin_vgg_v0.1.pb'
...
class MetricHarness:
  ...
  def __init__(
    ...
    if disable_lpips:
      ...
      self.image0_ph = tf.compat.v1.placeholder(tf.float32)
      self.image1_ph = tf.compat.v1.placeholder(tf.float32)

      # def lpips_fn(x, y):
      #   return lpips_tf.lpips(x, y, model='net-lin', net='vgg')
      self.lpips_fn = True  
      self.distance_t = lpips_tf.lpips(self.image0_ph, self.image1_ph, model='net-lin', net='vgg')

  def compute_lpips(self, image0_ph, image1_ph, distance_t, image0, image1, sess):
  ...
  def __call__(self, rgb_pred, rgb_gt, name_fn=lambda s: s):
    ...
    if self.lpips_fn is not None:
      ...
      with tf.compat.v1.Session(config=config) as session:
        metrics['lpips'] = self.compute_lpips(self.image0_ph, self.image1_ph, self.distance_t, rgb_gt[None], rgb_pred[None], session).squeeze()
