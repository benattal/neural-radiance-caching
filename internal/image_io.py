...
import tensorflow_graphics.io.exr
...
def load_exr(pth):
    ...
    image_data = tfg.io.exr.read_exr(pth)
    image = np.array(image_data[0], dtype=np.float32)
    return image
...
def save_img_u8(img, pth):
    ...
    tmp_pth = pth + ".tmp"
    with utils.open_file(tmp_pth, "wb") as f:
        img.save(f, "PNG")
    utils.mv_file(tmp_pth, pth)
