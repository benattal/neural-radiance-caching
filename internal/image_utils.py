...
def shift_invariant_ssim(img0, img1, *args):
    ...
    opt_score, opt_di, opt_dj = compute_shift_invariant_metric(img0, img1, score_fn, "argmax", *args)
...
def shift_invariant_mse(img0, img1, *args):
    ...
    opt_score, opt_di, opt_dj = compute_shift_invariant_metric(img0, img1, err_fn, "argmin", *args)
    opt_score = jnp.mean(opt_score)
    return opt_score, opt_di, opt_dj


def linear_to_srgb(linear, eps=None, xnp=jnp):
...
def srgb_to_linear(srgb, eps=None, xnp=jnp):
...
def precompute_nlinear_weights(coords, grid_shape):
    ...
    if coords.shape[-1] != len(grid_shape):
        raise ValueError(f"`coord` dim {coords.shape} does not match grid dim {len(grid_shape)}")
    ...
    idx0 = jnp.maximum(
        0,
        jnp.minimum(jnp.floor(coords).astype(jnp.int32), jnp.array(grid_shape) - 1),
    )
    ...
    for bitstring in list(itertools.product([0, 1], repeat=len(grid_shape))):
        ...
        weights.append(jnp.prod(weight0 * (1 - 2 * jnp.array(bitstring)) + jnp.array(bitstring), 1))
...
def correct_local_color(
    ...
    *,
    ...
    coords_spatial = jnp.stack(
        jnp.meshgrid(
            *[jnp.linspace(0, r - 1, s) for s, r in zip(im.shape[:-1], num_spatial_bins)],
            indexing="ij",
        ),
        axis=-1,
    )
...
class MetricHarness:
    ...
    def __init__(
        ...
        if disable_search_invariant:
            ...
            self.si_mse_fn = jax.jit(
                lambda x, y: shift_invariant_mse(x, y, search_radii, window_halfwidth)[  # pylint: disable=g-long-lambda
                    0
                ]
            )
    ...
    def __call__(self, rgb_pred, rgb_gt, name_fn=lambda s: s):
        ...
        if self.si_mse_fn is not None:
            metrics["psnr_si" + self.si_suffix] = mse_to_psnr(self.si_mse_fn(rgb_pred, rgb_gt))
        ...
        if ("psnr" in metrics) and ("ssim" in metrics) and ("lpips" in metrics):
            ...
            lpips = metrics["lpips"]
            metrics["avg_err"] = jnp.exp(jnp.mean(jnp.log(jnp.array([mse, sqrt_dssim, lpips]))))
