...
from internal import ref_utils, utils
...
def interlevel_loss(ray_history, *, mults):
  ...
  w = jax.lax.stop_gradient(ray_history[-1]['weights'] * ray_history[-1]['lossmult'])
  losses_interlevel = []
  for mult, ray_results in zip(mults, ray_history[:-1]):
    cp = ray_results['sdist']
    wp = ray_results['weights'] * ray_results['lossmult']
...
def spline_interlevel_loss(ray_history, *, mults, blurs, eps=1e-5):
  ...
  w = ray_history[-1]['weights'] * ray_history[-1]['lossmult']
  losses_interlevel = []
...
def distortion_loss(
  ...
  w = last_ray_results['weights'] * last_ray_results['lossmult']
...
def orientation_loss(
  ...
  w = ray_results['weights'] * ray_results['lossmult']
  ...
  n = jnp.nan_to_num(ray_results[target])
...
def predicted_normal_loss(
    ray_results, beta, *, coarse_mult=1.0, mult=1.0,
    gt='normals', pred='normals_pred', normalize=False, stopgrad=False, stopgrad_weight=1.0,
):
  ...
  if stopgrad:
    ...
    w = utils.stopgrad_with_weight(w, stopgrad_weight)

  n = jnp.nan_to_num(ray_results[gt])
  n = jax.lax.stop_gradient(n)
  n_pred = jnp.nan_to_num(ray_results[pred])
