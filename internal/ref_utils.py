...
def l2_normalize(x, grad_eps=jnp.finfo(jnp.float32).eps, tiny=jnp.finfo(jnp.float32).tiny):
  ...
  normal = math.override_gradient(normal_val, normal_grad)
...
def compute_weighted_mae(weights, normals, normals_gt):
  ...
  angles = math.safe_arccos((normals * normals_gt).sum(axis=-1))
