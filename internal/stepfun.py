...
def sample_intervals(
  ...
  samples = jnp.sort(jnp.clip(samples, *domain), axis=-1)
...
def lossfun_distortion(t, w, normalize=False):
  ...
  # ut = (t[Ellipsis, 1:] + t[Ellipsis, :-1]) / 2
  # ut_mean = (w * ut).mean(axis=-1, keepdims=True) / (jnp.sum(w, axis=-1, keepdims=True) + 1e-5)
  # t_0 = t[..., :-1]
  # t_1 = t[..., 1:]

  # loss_var = jax.lax.stop_gradient(w) * (
  #     (t_0 - ut_mean) ** 2 + (t_0 - ut_mean) * (t_1 * ut_mean) + (t_1 - ut_mean) ** 2
  # ) / 3.0

  # return loss_var.sum(axis=-1)
