...
def extract_transient_frames(path, composited=True, bkgd=None, gamma=2, scale=1, img_gamma=2, img_scale=1,
    ...
    if "transient" in out_folder:
        target_folder = "cache_time_slice"
        extension = "*.npy"
    ...
    for ind, file_fname in tqdm(enumerate(file_fnames)):
        ...
        elif "npy" in file_fname and "cache_time_slice" in target_folder:
            img = np.load(file_fname).squeeze() * time_mult
        else:
        ...
        if "transient" in out_folder:
            ...
            if "red" in out_folder:
                out[..., 0] = img.squeeze(axis=-1)
            if "green" in out_folder:
                out[..., 1] = img.squeeze(axis=-1)
            if "blue" in out_folder:
                out[..., 2] = img.squeeze(axis=-1)
            img = out
...
if __name__=="__main__":
    ...
    if "spheres" in queue:
        ...
        for i, c in enumerate(channels):
            ...
            #     extract_transient_frames(f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light', bkgd=f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light',
            #     composited=True, gamma=1/2, scale=0.1, img_scale=0.8, img_gamma=1/2, alpha=0.8, out_folder=f'../image_folders_moving/spheres_{c}_transient', time_mult=1.0)

            if "transient" in types:
                extract_transient_frames(f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light', bkgd=f'{moving_folder}/spheres_material_light_from_scratch_resample_eval_fixed_light',
                composited=True, gamma=1/2, scale=0.1, img_scale=0.8, img_gamma=1/2, alpha=0.8, out_folder=f'../image_folders_moving/spheres_{c}_transient', time_mult=1.0)

            # if "time_slice" in types:
            #     extract_transient_frames(f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light', bkgd=f'{moving_folder}/spheres_material_light_resample_eval_fixed_light',
            #     # composited=True, gamma=1/2, scale=0.2, img_scale=0.8, img_gamma=1/2, alpha=0.8, out_folder=f'../image_folders_moving/spheres_{c}_time_slice', time_slice=550, time_range=1, time_mult=1)
            #     composited=True, gamma=1/2, scale=0.2, img_scale=0.8, img_gamma=1/2, alpha=0.8, out_folder=f'../image_folders_moving/spheres_{c}_time_slice', time_slice=750, time_range=1, time_mult=1)
            
            if "direct" in types:
                extract_transient_frames(f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light',
                composited=False, gamma=1, scale=1, img_scale=0.8, img_gamma=1/2, alpha=0.9, out_folder=f'../image_folders_moving/spheres_{c}_direct')

            if "indirect" in types:
                extract_transient_frames(f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light',
                composited=False, gamma=1, scale=1.0, img_scale=0.8, img_gamma=1/2, alpha=0.9, out_folder=f'../image_folders_moving/spheres_{c}_indirect')

            # if "intensity" in types:
            #     extract_transient_frames(f'{moving_folder}/spheres_material_light_resample_finetune_{i}_eval_fixed_light', bkgd=f'{moving_folder}/spheres_material_light_resample_eval_fixed_light',
            #     composited=False, gamma=1, scale=1, img_scale=0.8, img_gamma=1/2, alpha=0.9, out_folder=f'../image_folders_moving/spheres_{c}_intensity')

        combine_into_rgb('../image_folders_moving/spheres', types)
